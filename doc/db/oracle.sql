...
comment on column XXL_JOB_REGISTRY.update_time
  is '修改时间';
-- Create/Recreate indexes
create index IDX_REGISTRY_GROUP on XXL_JOB_REGISTRY (REGISTRY_GROUP)
  ;
create index IDX_UPDATE_TIME on XXL_JOB_REGISTRY (UPDATE_TIME)
  ;
-- Create/Recreate primary, unique and foreign key constraints
...
alter table XXL_JOB_LOG_REPORT
  add constraint PK_XXL_JOB_LOG_REPORT_ID primary key (ID);
